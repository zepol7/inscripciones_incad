...
function calcular_valor_cuota(){
    var valor_tasa = $('#valor_tasa').val();
    var num_cuotas = $('#num_cuotas').val();
    var valor_financiar = replaceAll($('#valor_financiar').val(), ".", "" );  
    var cuota = 0;
    
    if(valor_tasa > 0){
        if(num_cuotas == 0 || valor_financiar == 0){        
            alert("Debe diligencia el numero de cuotas y el valor a financiar");
        }
        else{
            var tasa = valor_tasa/100;
            var cuota = (valor_financiar * tasa) / (1 - (Math.pow((1 + tasa),(num_cuotas * (-1)))) );
        ...
        else if(valor_financiar > 0){
            var cuota = (valor_financiar) / (num_cuotas);
            cuota = Math.round(cuota);
        }
        else{
            cuota = 0;
...
function calcular_periodicidad_cuota(id){
    
    var val_periodicidad = $(id).val();
    
    
    switch (val_periodicidad) {
        case 49://Semanal
...
function buscar_registro_id(){
	...
	//var params = 'opcion=2&txt_busca_id=' + txt_busca_id;
        var params = 'opcion=14&txt_busca_id=' + txt_busca_id;

    llamarAjax("registro_ajax.php", params, "principal_registro", "");
	
}

function cargar_registro_id(documento_persona){
	
	var params = 'opcion=2&txt_busca_id=' + documento_persona;
...
function cargar_firmar_documentos(documento_persona){	
	var params = 'opcion=1&txt_busca_id=' + documento_persona;     
    llamarAjax("registro_firma_ajax.php", params, "principal_registro", "");	
}

let documentos = [];

function agregarDocumento(tipo, archivo_64) {
  documentos.push({
    tipo: tipo,
    archivo_64: archivo_64
  });
}
...
/*
function firmar_documentos(id_academica) {	


    // lista de tipos de documento que quieres obtener
    var tipos = new Array();         
    $("input[name='check_archivo_pdf']:checked").each(function () {
        tipos.push($(this).val());
    }); 


    $('#contenedor_boton_'+id_academica).slideDown(600).css('display', 'none')
    $('#contenedor_cargando_'+id_academica).slideDown(600).css('display', 'block')        
    
    // Recorremos cada tipo y hacemos fetch   

    tipos.forEach(tipo => {        
        
        //fetch('pdf.php?id_registro=' + id_academica + '&tipo=' + tipo + '&modo=1')
        fetch('pdf.php?id_registro=' + id_academica + '&tipo=' + tipo + '&modo=1')
        .then(response => response.text())   // porque la salida es texto base64
        .then(base64 => {            

            agregarDocumento(tipo, base64);
            console.log("Documento agregado:", tipo);

            // Si ya tenemos todos los documentos, procesamos el JSON
            if (documentos.length === tipos.length) {
                console.log("Todos los documentos listos:", documentos);
                // Convertir a JSON
                let documentosJSON = JSON.stringify(documentos);     
                let documentosParam = encodeURIComponent(documentosJSON);           
                //console.log("JSON final:", documentosJSON);

                // Enviar por Ajax a PHP
                var params = 'opcion=2&id_academica=' + id_academica + '&documentos=' + documentosParam;                
                llamarAjax("registro_firma_ajax.php", params, "panel_firma_" + id_academica, "");

                $('#contenedor_boton_'+id_academica).slideDown(600).css('display', 'block')
                $('#contenedor_cargando_'+id_academica).slideDown(600).css('display', 'none')    

            }
        })
        .catch(err => console.error("Error al obtener PDF tipo " + tipo + ":", err));
...
function firmar_documentos(id_academica) {
    ...
    // Array de promesas
    let promesas = tipos.map(tipo => {
        return fetch('pdf.php?id_registro=' + id_academica + '&tipo=' + tipo + '&modo=1')
            .then(response => response.text())
            .then(base64 => {
                agregarDocumento(tipo, base64);
                //documentos.push({ tipo: tipo, base64: base64 });
            })
            .catch(err => console.error("Error al obtener PDF tipo " + tipo + ":", err));
    });
    ...
    Promise.all(promesas).then(() => {
        ...
        // Mostrar botón y ocultar cargando
        $('#contenedor_boton_' + id_academica).show();
        $('#contenedor_cargando_' + id_academica).hide();
    });

    

    //var params = 'opcion=2&id_academica=' + id_academica + '&documentos=' + "4444545";
    //llamarAjax("registro_firma_ajax.php", params, "panel_firma_" + id_academica, "");
...
function eliminar_registro(id_registro){
	...
	llamarAjax("registro_ajax.php", params, "principal_registro", "validar_eliminar()");
...
function activar_registro(id_registro){
	...
	llamarAjax("registro_ajax.php", params, "principal_registro", "validar_activar()");
...
function validar_eliminar() {
    ...
    var hdd_exito = $('#hdd_eliminar_exito').val();
    
    texto_exito = 'Registro Eliminado con Exito <br /> ';
    ...
    if (hdd_exito > 0) {
        ...
        $('#contenedor_error').html('Error al Eliminar Registro');
...
function validar_activar(){
    ...
    texto_exito = 'Registro Activado con Exito <br /> ';
    ...
    if (hdd_exito > 0) {
        ...
        $("#contenedor_error").addClass("contenedor_error_visible");
        $('#contenedor_error').html('Error al Activar Registro');
...
function validar_crear_editar_personas(tipo_accion) {
   	...
   	$("#tipo_documento").removeClass("borde_error");
        ...
        $("#periodicidad_pago").removeClass("borde_error");
        
        $("#id_promotor").removeClass("borde_error");
        
        $("#estado_matriculado").removeClass("borde_error");
        
        $("#id_ultimo_estudio").removeClass("borde_error");
        $("#id_forma_pago").removeClass("borde_error");
        $("#id_entidad_financiera").removeClass("borde_error");
   	...
   	if ($('#tipo_documento').val() == '') { $("#tipo_documento").addClass("borde_error"); result = 1; }
        ...
        if ($('#id_ultimo_estudio').val() == '') { $("#id_ultimo_estudio").addClass("borde_error"); result = 1; }
        ...
        if ($('#id_forma_pago').val() == '') { $("#id_forma_pago").addClass("borde_error"); result = 1; }
        
        if ($('#id_entidad_financiera').val() == '') { $("#id_entidad_financiera").addClass("borde_error"); result = 1; }
        ...
        if ($('#periodicidad_pago').val() == '') { $("#periodicidad_pago").addClass("borde_error"); result = 1; }
        ...
        */
        ...
        if ($('#id_promotor').val() == '') { $("#id_promotor").addClass("borde_error"); result = 1; }
        
        if ($('#estado_matriculado').val() == '') { $("#estado_matriculado").addClass("borde_error"); result = 1; }
...
function crear_registro() {
    ...
    llamarAjax("registro_ajax.php", params, "principal_registro_create_edit", "validar_exito();");
    //llamarAjax("registro_ajax.php", params, "principal_registro_create_edit", "");
...
function crear_registro_credito() {
    ...
    llamarAjax("registro_credito_ajax.php", params, "principal_registro_create_edit", "validar_exito();");
    //llamarAjax("registro_ajax.php", params, "principal_registro_create_edit", "");
...
function validar_sin_info(id){
    var check_estado = $(id).is(':checked') ? 1 : 0;
    
    if(check_estado == 1){
        
        $('#tipo_documento_codeudor').val(3);
        $('#documento_codeudor').val("NA");
        $('#nombre_codeudor').val("NA");
        $('#apellido_codeudor').val("NA");
        $('#fecha_nacimiento_codeudor').val("01/01/1999");
        $('#edad_codeudor').val('99');
        $('#direccion_residencia_codeudor').val("NA");
        $('#barrio_residencia_codeudor').val("NA");
        $('#ciudad_residencia_codeudor').val("NA");
        $('#tel_casa_codeudor').val("NA");
        $('#tel_movil_codeudor').val("NA");
        $('#email_codeudor').val("NA");
        $('#actividad_economica_codeudor').val("NA");
        $('#ingreso_mensual_codeudor').val(0);
        $('#nombre_empresa_codeudor').val("NA");
        $('#direccion_empresa_codeudor').val("NA");
        $('#telefono_empresa_codeudor').val("NA");
        $('#tipo_vehiculo_codeudor').val(48);
        $('#placa_vehiculo_codeudor').val("NA");
        $('#marca_vehiculo_codeudor').val("NA");
        $('#modelo_vehiculo_codeudor').val("NA");
        $('#nom_ref_familiar_uno_codeudor').val("NA");
        $('#tel_ref_familiar_uno_codeudor').val("NA");
        $('#nom_ref_familiar_dos_codeudor').val("NA");
        $('#tel_ref_familiar_dos_codeudor').val("NA");
        $('#nom_ref_personal_uno_codeudor').val("NA");
        $('#tel_ref_personal_uno_codeudor').val("NA");
        $('#nom_ref_personal_dos_codeudor').val("NA");
        $('#tel_ref_personal_dos_codeudor').val("NA");
    }
    else{
        $('#tipo_documento_codeudor').val(0);
        $('#documento_codeudor').val("");
        $('#nombre_codeudor').val("");
        $('#apellido_codeudor').val("");
        $('#fecha_nacimiento_codeudor').val();
        $('#edad_codeudor').val("");
        $('#direccion_residencia_codeudor').val("");
        $('#barrio_residencia_codeudor').val("");
        $('#ciudad_residencia_codeudor').val("");
        $('#tel_casa_codeudor').val("");
        $('#tel_movil_codeudor').val("");
        $('#email_codeudor').val("");
        $('#actividad_economica_codeudor').val("");
        $('#ingreso_mensual_codeudor').val(0);
        $('#nombre_empresa_codeudor').val("");
        $('#direccion_empresa_codeudor').val("");
        $('#telefono_empresa_codeudor').val("");
        $('#tipo_vehiculo_codeudor').val(48);
        $('#placa_vehiculo_codeudor').val("");
        $('#marca_vehiculo_codeudor').val("");
        $('#modelo_vehiculo_codeudor').val("");
        $('#nom_ref_familiar_uno_codeudor').val("");
        $('#tel_ref_familiar_uno_codeudor').val("");
        $('#nom_ref_familiar_dos_codeudor').val("");
        $('#tel_ref_familiar_dos_codeudor').val("");
        $('#nom_ref_personal_uno_codeudor').val("");
        $('#tel_ref_personal_uno_codeudor').val("");
        $('#nom_ref_personal_dos_codeudor').val("");
        $('#tel_ref_personal_dos_codeudor').val("");
    }
