...
//------    Máxima cifra soportada: 18 dí­gitos con 2 decimales
//------    999,999,999,999,999,999.99 
// NOVECIENTOS NOVENTA Y NUEVE MIL NOVECIENTOS NOVENTA Y NUEVE BILLONES 
//	NOVECIENTOS NOVENTA Y NUEVE MIL NOVECIENTOS NOVENTA Y NUEVE MILLONES 
//	NOVECIENTOS NOVENTA Y NUEVE MIL NOVECIENTOS NOVENTA Y NUEVE PESOS 99/100 M.N.
//------		PHP Version 4.3.1 o mayores (aunque podrí­a funcionar en versiones anteriores, tendrí­as que probar)
...
for($xz = 0; $xz < 3; $xz++)
	...
	$xi = 0; $xlimite = 6; // inicializo el contador de centenas xi y establezco el lí­mite a 6 dí­gitos en la parte entera
	$xexit = true; // bandera para controlar el ciclo del While	
	while ($xexit)
		{
		if ($xi == $xlimite) // si ya llegó al lí­mite m&aacute;ximo de enteros
			{
			break; // termina el ciclo
			}
	
		$x3digitos = ($xlimite - $xi) * -1; // comienzo con los tres primeros digitos de la cifra, comenzando por la izquierda
		$xaux = substr($xaux, $x3digitos, abs($x3digitos)); // obtengo la centena (los tres dí­gitos)
		for ($xy = 1; $xy < 4; $xy++) // ciclo para revisar centenas, decenas y unidades, en ese orden
			{
			switch ($xy) 
				{
				case 1: // checa las centenas
					if (substr($xaux, 0, 3) < 100) // si el grupo de tres dí­gitos es menor a una centena ( < 99) no hace nada y pasa a revisar las decenas
						{
						}
					else
						{
		...
		// ----------- esta lí­nea la puedes cambiar de acuerdo a tus necesidades o a tu paí­s -------
		if (trim($xaux) != "")
			{
			switch ($xz)
				{
